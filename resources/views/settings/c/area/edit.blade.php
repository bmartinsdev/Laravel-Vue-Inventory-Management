	...
	<div class="main">
		<div class="breadcrumb-holder d-flex">
			...
			<div class="flex-shrink-1 pt-2">
				<a onClick="abrirmodal({{ $area->id }})" class="grid-btn bgc-red">Apagar</a>
			</div>
		</div>
		<div class="card p-4 mt-4">
			<h2 class="mb-3 border-bottom brdc-blue">Editar local</h2>
			<form action="/definicoes/c/locais/{{ $area->id }}" method="POST">
				@csrf
				@method('PUT')
				<div class="form-group">
					<label for="inputName">Nome</label>
					<input type="text" class="form-control @error('nome') is-invalid @enderror" id="inputName" name="nome"
						value="{{ old('nome', $area->nome) }}" aria-describedby="nameHelp" placeholder="Insira nome" required>
					@error('nome')
